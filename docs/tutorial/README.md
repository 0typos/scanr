...
for the services, rootless podman for the proxies, a user-space `sshd` for the tunnel.
...
`scanr-lab up` then works from anywhere; `scanr-lab uninstall` removes the symlink. The
first run fetches `typer` into uv's cache; every run after is instant.
