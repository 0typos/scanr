...
- Amended: `--tls-versions` asks each version for itself. A server answers with the highest version it shares, so one hello never reveals that SSLv3 is still spoken; the survey sends a hello of each era — SSLv2 CLIENT-HELLO, an extension-free SSLv3 hello, SNI-only 1.0 and 1.1 hellos, and a 1.2-only hello when 1.3 was taken — each on its own connection, up to five per silent open port, opt-in and needing `tls`. The record says which versions are accepted, `legacy_only` when the newest is older than 1.2, and `advice` naming the client that can still reach it; the line says `versions:ssl3..1.3` or `legacy-only:tls1.0`. An SSLv2 SERVER-HELLO's certificate stands in for the leaf when nothing newer answered. Cipher enumeration stays out: that is `testssl`'s job, and dozens of connections per port.
